...
setwd("I:/psivakumar/Emer_CH/gwas/uk_swedish_alt/SAIGE_results")
...
#remove spurious chr5 hit
chr5.out <- filter(saige.df, CHR == 5, p.value < 1E-5)$SNPID
saige.df <- saige.df[!saige.df$SNPID %in% chr5.out, ]
#fwrite(saige.df, "imputed_all_results.csv")
saige.sig <- filter(saige.df, p.value < 10E-6)
...
#ggsave("manhattan_filt.png", manhattan, device = "png", dpi = 600)

plotting.full <- res_log10 %>%
  group_by(CHR) %>%
  summarise(chr_len=max(POS)) %>%
  mutate(tot = cumsum(as.numeric(chr_len)) - chr_len) %>%
  dplyr::select(-chr_len) %>%
  left_join(res_log10, ., by = "CHR") %>%
  arrange(ordered(CHR), POS) %>%
  mutate(POScum=POS+tot)

axis_df.full <- plotting.full %>%
  group_by(CHR) %>%
  summarise(center = (max(POScum) + min(POScum)) / 2)

manhattan.full <-
  ggplot(plotting.full, aes(POScum, log10P)) +
  geom_point(aes(colour=as.factor(CHR)), alpha = 0.8, size = 1.3) +
  scale_color_manual(values = rep(c("light grey", "dark grey"), 22)) +
  scale_x_continuous(label = axis_df.full$CHR, breaks = axis_df.full$center) +
  scale_y_continuous(expand = c(0, 0.05)) +
  theme_classic() +
  labs(x = "CHR", y = "-log10P") +
  theme(legend.position = "none") +
...
#ggsave("manhattan_full.png", manhattan.full, device = "png", dpi = 600)
...
#ggsave("qqplot_full.png", qqplt, device = "png", dpi = 600)
...
#  fwrite(locuszoom_data[[i]], paste0("I:/psivakumar/Emer_CH/gwas/uk_swedish_alt/locuszoom/lz_chr", chr.order[i], ".txt", sep = ""), sep = '\t')
#}

#unimputed manhattan
unimp.model <- fread("I:/psivakumar/Emer_CH/gwas/uk_swedish_alt/unimputed_fishers/unimputed_fishers.model")
...
#ggsave("I:/psivakumar/Emer_CH/gwas/uk_swedish_alt/unimputed_fishers/unimputed_manhattan.png", unimp.manhattan, device = "png", dpi = 600)

unimp.qq <- rename(unimp.model.plotting, "p.value" = P) %>% qqplotter()

#ggsave("I:/psivakumar/Emer_CH/gwas/uk_swedish_alt/unimputed_fishers/unimputed_qqplot.png", unimp.qq, device = "png", dpi = 600)

unimp.chisq <- qchisq(1-unimp.model.plotting$P, 1)
unimp.lambda <- median(unimp.chisq) / qchisq(0.5,1)
#write.table(unimp.lambda, "I:/psivakumar/Emer_CH/gwas/uk_swedish_alt/unimputed_fishers/unimputed_lambda.txt")
...
#fwrite(unimp.sig, "I:/psivakumar/Emer_CH/gwas/uk_swedish_alt/unimputed_fishers/unimputed_sig.csv")

unimp.lz.data <- unimp.model.plotting %>% dplyr::select("CHR", "POS", "P") %>% unite("ID", c("CHR", "POS"), sep = ":") %>% arrange(P)
#fwrite(unimp.lz.data, "I:/psivakumar/Emer_CH/gwas/uk_swedish_alt/locuszoom/unimputed_lz_data.txt", sep = '\t')
