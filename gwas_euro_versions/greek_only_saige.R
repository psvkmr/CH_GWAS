...
setwd("I:/psivakumar/Emer_CH/gwas/greek_only_gwas/SAIGE_results")
...
#  fwrite(locuszoom_data[[i]], paste0("I:/psivakumar/Emer_CH/gwas/greek_only_gwas/locuszoom/lz_chr", chr.order[i], ".txt", sep = ""), sep = '\t')
#}

#unimputed manhattan
unimp.model <- fread("I:/psivakumar/Emer_CH/gwas/greek_only_gwas/unimputed_fishers/unimputed_fishers.model")
...
#write.table(unimp.lambda, "I:/psivakumar/Emer_CH/gwas/greek_only_gwas/unimputed_fishers/unimputed_lambda.txt")
...
#fwrite(unimp.sig, "I:/psivakumar/Emer_CH/gwas/greek_only_gwas/unimputed_fishers/unimputed_sig.csv")

unimp.lz.data <- unimp.model.plotting %>% dplyr::select("CHR", "POS", "P") %>% unite("ID", c("CHR", "POS"), sep = ":") %>% arrange(P)
#fwrite(unimp.lz.data, "I:/psivakumar/Emer_CH/gwas/greek_only_gwas/locuszoom/unimputed_lz_data.txt", sep = '\t')


# compare to uk results 

uk.sig.res <- fread("I:/psivakumar/Emer_CH/gwas/uk_analysis/SAIGE_results/imputed_sig_results_e5_uk_rsIDs.tsv")
greek.res.with.uk.sig <- inner_join(saige.df, uk.sig.res, by = "SNPID")
greek.of.int <- filter(greek.res.with.uk.sig, sign(BETA.x) == sign(BETA.y))
ggplot(greek.res.with.uk.sig, aes(BETA.x, BETA.y)) + geom_point() + theme_classic()

uk.lead.snps <- c("rs4519530", "rs6435024", "rs9386670")
greek.uk.lead.snps <- filter(greek.of.int, ID %in% uk.lead.snps)
