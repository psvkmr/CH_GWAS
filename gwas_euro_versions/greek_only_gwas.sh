...
# rename samples
awk '{print $2,"GSA2019_"$1"_"$2,$3,$4,$5,$6}' gsa2019_4_commonSnps.fam > rename_GSA2019.txt
mv rename_GSA2019.txt gsa2019_4_commonSnps.fam
# edit fam file to include phenotypes in R
# check for any strand errors
plink --bfile gsa2019_4_commonSnps --allow-no-sex --flip-scan --out postMerge
# create list of strand issue variants to remove in R
plink --bfile gsa2019_4_commonSnps --exclude strandIssueSnpsToRemove.txt --allow-no-sex --make-bed --recode --out all_6_noStrandIssue
...
# edited 2 duplicates in fam, will be removed anyway
sed -e '0,/255_D1719190/ s/255_D1719190/255_D1719190_edit/' all_6_noStrandIssue.fam > tmp
sed -e '0,/333_D1719873/ s/333_D1719873/333_D1719873_edit/' tmp > all_6_noStrandIssue.fam
...
plink --bfile controls_8_snps95 --mind 0.05 --make-bed --out controls_9_ind98
plink --bfile cases_8_snps95 --mind 0.05 --make-bed --out cases_9_ind98
# sex check
...
for i in `seq 1 22`
do
        unzip -PI5EHmKdecmNLN1 chr_${i}.zip &
done
unzip chr_X.zip
# check sample orders
...
Rscript /hades/Software/NGS_Software/SAIGE/extdata/step1_fitNULLGLMM.R \
        --plinkFile=/hades/psivakumar/Emer_CH/greek_only_gwas/saige_all_26 \
        --phenoFile=/hades/psivakumar/Emer_CH/greek_only_gwas/saige.pheno \
        --phenoCol=Phenotype \
        --covarColList=PC1,PC2,PC3,PC4,PC5,PC6,PC7,PC8,PC9,PC10,PC11,PC12,PC13,PC14,PC15,PC16,PC17,PC18,PC19,PC20 \
        --sampleIDColinphenoFile=IID \
        --traitType=binary \
        --skipModelFitting=FALSE \
        --outputPrefix=/hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step1 \
        --nThreads=2 \
        &> /hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step1.log

# step 2
Rscript /hades/Software/NGS_Software/SAIGE/extdata/step2_SPAtests.R \
        --vcfFile=/hades/psivakumar/Emer_CH/greek_only_gwas/chr22.imputed.filt.vcf.gz \
        --vcfFileIndex=/hades/psivakumar/Emer_CH/greek_only_gwas/chr22.imputed.filt.vcf.gz.tbi \
        --vcfField=DS \
        --chrom=22 \
        --minMAC=1 \
        --sampleFile=/hades/psivakumar/Emer_CH/greek_only_gwas/saige.sample \
        --GMMATmodelFile=/hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step1.rda \
        --varianceRatioFile=/hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step1.varianceRatio.txt \
        --SAIGEOutputFile=/hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step2_output.txt \
        --numLinesOutput=2 \
        --IsOutputAFinCaseCtrl=TRUE \
        &> /hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step2.log
# as loop
#!/bin/bash
saige_step2(){
  Rscript /hades/Software/NGS_Software/SAIGE/extdata/step2_SPAtests.R \
          --vcfFile=/hades/psivakumar/Emer_CH/greek_only_gwas/chr$1.imputed.filt.vcf.gz \
          --vcfFileIndex=/hades/psivakumar/Emer_CH/greek_only_gwas/chr$1.imputed.filt.vcf.gz.tbi \
          --vcfField=DS \
          --chrom=$1 \
          --minMAC=1 \
          --sampleFile=/hades/psivakumar/Emer_CH/greek_only_gwas/saige.sample \
          --GMMATmodelFile=/hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step1.rda \
          --varianceRatioFile=/hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step1.varianceRatio.txt \
          --SAIGEOutputFile=/hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step2_chr$1_output.txt \
          --numLinesOutput=2 \
          --IsOutputAFinCaseCtrl=TRUE
}
...
for i in `seq 1 22`
do
    saige_step2 ${i} &> /hades/psivakumar/Emer_CH/greek_only_gwas/SAIGE_step2_chr${i}.log &
done
