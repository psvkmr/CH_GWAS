...
# merge
plink --bfile gsa2019_4_commonSnps --bmerge ich_4_commonSnps --merge-mode 4 --out all_5_merged
...
plink --bfile all_6_noStrandIssue --keep euroCases.txt --make-bed --recode --out cases_7_split
plink --bfile all_6_noStrandIssue --keep euroControls.txt --make-bed --recode --out controls_7_split
...
awk '$10>0.98 {print $2,$4,$10}' relCheck.genome > duplicateInfo.txt
awk '$10>0.98 {print $1,$2}' relCheck.genome | sed '1d' > duplicateSamples.txt
...
plink --bfile all_24_hhMissing --fisher --model --out unimputed_fishers/unimputed_fishers
# get final freq file
...
for i in `seq 1 22`
do
        unzip -P "VnT3sswo|KCX#6" chr_${i}.zip &
done
unzip chr_X.zip
# check sample orders
...
for i in `seq 1 22`;
do
tabix -p vcf chr${i}.imputed.poly.vcf.gz 2> err/chr${i}.imputed.poly.tabix.err &
done
# saige prep
...
/hades/Software/NGS_Software/plink_linux_x86_64_dev/plink --bfile all_24_hhMissing --make-set highLDRegionsIds.txt --write-set --out highLDR_v2
/hades/Software/NGS_Software/plink_linux_x86_64_dev/plink --bfile all_24_hhMissing --exclude highLDR_v2.set  --make-bed --out all_25_nohighLDR
...
Rscript /hades/Software/NGS_Software/SAIGE/extdata/step1_fitNULLGLMM.R \
        --plinkFile=/hades/psivakumar/Emer_CH/euro_gwas/saige_all_26 \
        --phenoFile=/hades/psivakumar/Emer_CH/euro_gwas/saige.pheno \
        --phenoCol=Phenotype \
        --covarColList=PC1,PC2,PC3,PC4,PC5,PC6,PC7,PC8,PC9,PC10,PC11,PC12,PC13,PC14,PC15,PC16,PC17,PC18,PC19,PC20 \
        --sampleIDColinphenoFile=IID \
        --traitType=binary \
        --skipModelFitting=FALSE \
        --outputPrefix=/hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step1 \
        --nThreads=2 \
        &> /hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step1.log

# step 2
#Rscript /hades/Software/NGS_Software/SAIGE/extdata/step2_SPAtests.R \
#        --vcfFile=/hades/psivakumar/Emer_CH/euro_gwas/chr22.imputed.poly.vcf.gz \
#        --vcfFileIndex=/hades/psivakumar/Emer_CH/euro_gwas/chr22.imputed.poly.vcf.gz.tbi \
#        --vcfField=DS \
#        --chrom=22 \
#        --minMAC=1 \
#        --sampleFile=/hades/psivakumar/Emer_CH/euro_gwas/saige.sample \
#        --GMMATmodelFile=/hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step1.rda \
#        --varianceRatioFile=/hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step1.varianceRatio.txt \
#        --SAIGEOutputFile=/hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step2_output.txt \
#        --numLinesOutput=2 \
#        --IsOutputAFinCaseCtrl=TRUE \
#        &> /hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step2.log

# as loop
#!/bin/bash
saige_step2(){
  Rscript /hades/Software/NGS_Software/SAIGE/extdata/step2_SPAtests.R \
          --vcfFile=/hades/psivakumar/Emer_CH/euro_gwas/chr$1.imputed.poly.vcf.gz \
          --vcfFileIndex=/hades/psivakumar/Emer_CH/euro_gwas/chr$1.imputed.poly.vcf.gz.tbi \
          --vcfField=DS \
          --chrom=$1 \
          --minMAC=1 \
          --sampleFile=/hades/psivakumar/Emer_CH/euro_gwas/saige.sample \
          --GMMATmodelFile=/hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step1.rda \
          --varianceRatioFile=/hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step1.varianceRatio.txt \
          --SAIGEOutputFile=/hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step2_chr$1_output.txt \
          --numLinesOutput=2 \
          --IsOutputAFinCaseCtrl=TRUE
}
...
for i in `seq 1 22`
do
    saige_step2 ${i} &> /hades/psivakumar/Emer_CH/euro_gwas/SAIGE_step2_chr${i}.log &
done
