...
euro.samples.list <- list(belgian.cases, danish.cases)
...
euro.cases <- unlist(bind_rows(belgian.cases, danish.cases), use.names = F) %>% .[!. %in% unlist(missing.euro.samples, use.names = F)] %>% .[!duplicated(.)]
euro.controls <- unlist(ich.controls.fam[, "V2"], use.names = F) %>% .[!. %in% unlist(missing.euro.samples, use.names = F)] %>% .[!duplicated(.)]
...
setwd("I:/psivakumar/Emer_CH/gwas/euro_uk_no_esp_gre")
...
unknown.included <- filter(pop.strat, `ancestry-prediction` == "UNKNOWN")$sample_id
