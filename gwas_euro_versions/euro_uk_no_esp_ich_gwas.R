...
euro.samples.list <- list(belgian.cases, danish.cases, greek.cases, greek.controls)
...
euro.cases <- unlist(bind_rows(belgian.cases, danish.cases, greek.cases), use.names = F) %>% .[!. %in% unlist(missing.euro.samples, use.names = F)] %>% .[!duplicated(.)]
euro.controls <- unlist(greek.controls, use.names = F) %>% .[!. %in% unlist(missing.euro.samples, use.names = F)] %>% .[!duplicated(.)]

euro.cases.fam <- filter(gsa2019.fam, V2 %in% euro.cases)[, 1:2]
euro.controls.fam <- filter(gsa2019.fam, V2 %in% euro.controls)[, 1:2]
...
#fwrite(gsa2019.samples.other, "I:/psivakumar/Emer_CH/gwas/euro_uk_no_esp_ich/unknown_gsa2019_ids_remove.txt", col.names = F, sep = '\t')

# merge uk and euro

fam.list <- list(gsa2016.fam, gsa2018.fam, gsa2019.fam, reseq.fam, nbs.fam, c1958.fam)
full.fam <- bind_rows(fam.list)
...
setwd("I:/psivakumar/Emer_CH/gwas/euro_uk_no_esp_ich/")
...
all.fam$V6 <- ifelse(all.fam$V1 %in% all.cases, 2, ifelse(all.fam$V1 %in% all.controls, 1, 0))
...
het.fail <- filter(all.hets, HET_RATE > 0.5 | HET_RATE < 0.1)[, 1:2]
...
info.fam <- all.20.fam %>%
  separate(V2, c("old_WTC_ID", "dataset", "old_FID", "old_IID"), sep = "_", remove = F, fill = "left") %>%
  #mutate(pheno = ifelse(old_IID %in% euro.cases, 2, 1)) %>%
  mutate(batch = ifelse(old_IID %in% euro.samples.list[[1]]$V1, yes = "belgian", no = 
                                   ifelse(old_IID %in% euro.samples.list[[2]]$V1, yes = "danish", no = 
                                            ifelse(old_IID %in% euro.samples.list[[3]]$V1, yes = "greek", no = 
                                                     ifelse(old_IID %in% euro.samples.list[[4]]$V1, yes = "greek_control", no = 
